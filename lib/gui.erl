...
do_init({WxServer, ChatServerName}) ->
    ...
    % Try to create unique names for the client and gui processes
    % (This approach introduces races, to simplify the implementation -- don't do this in production code!)
    ...
    Tab = create_tab(ClientID, ?SYSTEM_NAME, Msg),
    ...
    wxFrame:show(Frame),
    focus(join_ids(ClientID, ?CMDLINE)),
...
handle_event(#wx{ event = #wxCommand{type = command_text_enter, cmdString = Item} },
    ...
    clear_text(join_ids(ClientID, ?CMDLINE)),
    ...
    case Cmd  of
        ...
        {connect_remote, Server, Machine} ->
            write_channel(join_ids(ClientID, ?SYSTEM), "* "++"Trying to connect to "++Server++" in machine "++Machine++"..."),
            Result = catch_fatal (ClientID, Panel, fun () -> request(ClientName, {connect, {Server, Machine}}) end ),
            case Result of
                ok     -> write_channel(join_ids(ClientID, ?SYSTEM), "+ Connected!") ;
                error  -> ok
            end ;

        % Connecting to the server
        {connect, Server} ->
            write_channel(join_ids(ClientID, ?SYSTEM), "* "++"Trying to connect to "++Server++"..."),
            Result = catch_fatal (ClientID, Panel, fun () -> request(ClientName, {connect, Server}) end ),
            case Result of
                ok     -> write_channel(join_ids(ClientID, ?SYSTEM), "+ Connected!") ;
                error  -> ok
            end ;
        ...
        disconnect ->
            Result = catch_fatal (ClientID, Panel, fun () -> request(ClientName, disconnect) end ),
            case Result of
                ok    -> write_channel(join_ids(ClientID, ?SYSTEM), "+ Disconnected") ;
                error -> ok
            end ;

        % Joining a new channel
        {join, Channel} ->
            Result = catch_fatal(ClientID, Panel, fun () -> request(ClientName, {join, Channel}) end ),
            case Result of
                ok -> write_channel(join_ids(ClientID, ?SYSTEM), "+ Joined "++Channel),
                      Tab = create_tab(ClientID, Channel, "* Channel "++Channel++"\n"),
                      label(ClientID, Channel, Tab) ;
                error  -> ok
            end ;

        % /leave
        leave ->
            Channel = active_channel(join_ids(ClientID,?NOTEBOOK)),
            leave_channel(ClientID, ClientName, Panel, Channel) ;
        ...
        {msg, String} ->
            Channel = active_channel(join_ids(ClientID,?NOTEBOOK)),
            case Channel of
                 ?SYSTEM_NAME ->  write_channel(join_ids(ClientID, ?SYSTEM), "- "++"Command not recognized: "++String) ;
                 _        -> Result = catch_fatal(ClientID, Panel,
                                                  fun () -> request(ClientName, {message_send, Channel, String}) end ),
                             case Result of
                                 ok    -> write_channel(join_ids(ClientID, Channel), String) ;
                                 error -> ok
                             end
            end ;
        ...
        % Who am I?
        whoami ->
            Result = catch_fatal(ClientID, Panel, fun () -> request(ClientName, whoami)  end),
            case Result of
                error -> ok ;
                Nick  -> write_channel(join_ids(ClientID, ?SYSTEM), "* "++"You are "++Nick)
            end ;

        % Change nickname
        {nick, Nick} ->
            Result = catch_fatal(ClientID, Panel, fun () -> request(ClientName,{nick, Nick}) end ),
            case Result of
                ok    -> write_channel(join_ids(ClientID, ?SYSTEM),
        ...
        % Ping someone
        {ping, Nick} ->
            Result = catch_fatal(ClientID, Panel, fun () -> request(ClientName,{ping, Nick}) end ),
            case Result of
                ok    -> write_channel(join_ids(ClientID, ?SYSTEM),
                         "* "++"Ping "++Nick) ;
                error -> ok
        ...
        {ignore, Line}    ->
            write_channel(join_ids(ClientID, ?SYSTEM), "- "++"Command not recognized"),
            write_channel(join_ids(ClientID, ?SYSTEM), Line)
    end,
    focus(join_ids(ClientID, ?CMDLINE)),
...
handle_event(#wx{ event = #wxAuiNotebook{type = command_auinotebook_button, selection = TabPos} },
             St = #state{ panel = Panel, client = ClientName, clientid = ClientID }) ->
    Ntbk    = typed_search(join_ids(ClientID, ?NOTEBOOK), wxAuiNotebook),
...
handle_call({message_receive, Channel, Msg}, _From, State = #state{ clientid = ClientID }) ->
    write_channel(join_ids(ClientID, Channel), Msg),
    {reply, ok, State} ;

% Here, the GUI receives a message to the system tab
handle_call({msg_to_SYSTEM, Msg}, _From, State = #state{ clientid = ClientID }) ->
    write_channel(join_ids(ClientID, ?SYSTEM), "* "++Msg),
    {reply, ok, State} ;
...
create_tab(ClientID, Title, Init) ->
    Ntbk = typed_search(join_ids(ClientID, ?NOTEBOOK), wxAuiNotebook),
...
% Sandro (2022-02-17): the functions 'with_label', 'join_ids' and
% 'channel_id' implemented conversions from client names, IDs and
% channel names to integers, which are bignums in Erlang.  The
% resulting integer was then used as an ID for wxWindows, which
% doesn't like integer IDs that don't fit into a 32-bit native
% integer, at least in newer versions of the API.  Since these
% functions were essentially computing a hash (albeit a variable-lenth
% one), I decided to just replace them with Erlang's built-in
% portlable hash: erlang:phash2/2.  I also refactored the code so that
% 'with_label' and 'channel_id' are no longer used at all.

%% % with_label("client_123", 9) = 1239
%% with_label(ClientName, Id) ->
%%     CIdStr = lists:sublist(ClientName,8,5),
%%     {CId, _} = string:to_integer(CIdStr),
%%     join_ids(CId, Id).
...
join_ids(ClientName, Id) -> erlang:phash2({ClientName, Id}).
...
% Errors
catch_fatal(ClientID, Panel, Cmd) ->
    case catch( Cmd() ) of
        {'EXIT',Reason} -> fatal_dialog(Panel, Reason) ;
        {error,_,Msg}   -> write_channel(join_ids(ClientID, ?SYSTEM), "- Error: "++Msg), error ;
        Result          -> Result
    end.
...
leave_channel(ClientID, ClientName, Panel, Channel) ->
    case Channel of
         ?SYSTEM_NAME -> write_channel(join_ids(ClientID, ?SYSTEM), "- "++"Cannot leave System tab") ;
         Channel -> Result = catch_fatal(ClientID, Panel,
                                         fun () -> request(ClientName, {leave, Channel}) end ),
                    case Result of
                         ok    -> close_tab(join_ids(ClientID, ?NOTEBOOK), Channel),
                                  write_channel(join_ids(ClientID, ?SYSTEM), "* "++"Left "++Channel) ;
                         error -> ok
                    end
    end.
